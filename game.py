...
class Game:
    ...
    action_event:asyncio.Event=field(default_factory=asyncio.Event,repr=False); discussion_seconds:int=120; night_seconds:int=120; last_word_seconds:int=30; vote_seconds:int=120
    bot_count:int=0; bot_difficulty:str="medium"; MIN_PLAYERS=4
    ...
    def reset_to_lobby(self,keep_players=False):
        ...
        if not keep_players:self.players.clear();self.player_names.clear();self.player_usernames.clear();self.bot_count=0
    ...
    def role_distribution(self):
        ...
        mc=max(1,round(len(self.players)*.25));r=[MAFIA]*mc
        if len(self.players)>=6:r += [COMMISSIONER]
        r += [DOCTOR]
