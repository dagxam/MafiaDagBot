...
async def start_handler(message: Message):
    ...
    if message.chat.type in (ChatType.GROUP, ChatType.SUPERGROUP):
        ...
        welcome_messages[chat_id] = welcome.message_id
...
async def bot_added_to_group(event: ChatMemberUpdated, bot: Bot):
    ...
    welcome = await bot.send_message(
        chat.id, get_welcome_text(chat.title or "MAFIA"),
        reply_markup=admin_start_keyboard(), parse_mode="HTML"
    )
    welcome_messages[chat.id] = welcome.message_id
    game_message_ids.setdefault(chat.id, set()).add(welcome.message_id)


async def create_lobby_for_chat(bot: Bot, chat_id: int, creator_id: int, group_title: str):
    game = games.get(chat_id)
    if game and game.started:
        return None, "⚠️ Игра уже идёт."
    if game and game.players:
        return None, "⚠️ Уже есть открытое лобби."
    await cleanup_game_messages(bot, chat_id)
    welcome_messages.pop(chat_id, None)
    game = Game(chat_id=chat_id, creator_id=creator_id, group_title=group_title or "MAFIA")
    games[chat_id] = game
    lobby = await bot.send_message(
        chat_id, get_lobby_text(game),
        reply_markup=lobby_keyboard(True, False), parse_mode="HTML"
    )
...
async def mafia_command_handler(message: Message, bot: Bot):
    if message.chat.type not in (ChatType.GROUP, ChatType.SUPERGROUP):
        await message.answer("🎭 Команду /mafia используйте в группе.")
        return
    if not await is_group_admin(bot, message.chat.id, message.from_user.id):
        await message.answer("⚠️ Только администратор группы может начать Мафию.")
        return
    game, error = await create_lobby_for_chat(bot, message.chat.id, message.from_user.id, message.chat.title or "MAFIA")
    if error:
        await message.answer(error)


@dp.message(Command("stop"))
async def stop_command_handler(message: Message, bot: Bot):
    if message.chat.type not in (ChatType.GROUP, ChatType.SUPERGROUP):
        await message.answer("⏹ Команду /stop используйте в группе.")
        return
    if not await is_group_admin(bot, message.chat.id, message.from_user.id):
        await message.answer("⚠️ Только администратор группы.")
        return
    chat_id = message.chat.id
    game = games.get(chat_id)
    if game:
...
async def restart_command_handler(message: Message, bot: Bot):
    if message.chat.type not in (ChatType.GROUP, ChatType.SUPERGROUP):
        await message.answer("🔄 Команду /restart используйте в группе.")
        return
    if not await is_group_admin(bot, message.chat.id, message.from_user.id):
        await message.answer("⚠️ Только администратор группы.")
        return
    ...
    if not game or not game.can_start():
        await message.answer("❌ Для рестарта нужно минимум 4 игрока в текущем лобби/игре.")
        return
    task = game_tasks.get(chat_id)
    if task and not task.done():
        task.cancel()
...
async def create_game_handler(callback: CallbackQuery, bot: Bot):
    ...
    game, error = await create_lobby_for_chat(bot, chat_id, uid, callback.message.chat.title or "MAFIA")
    if error:
...
async def start_game_handler(callback: CallbackQuery, bot: Bot):
    ...
    if not game: await callback.answer("❌ Сначала создайте игру через 🎭 НАЧАТЬ МАФИЮ или /mafia.", show_alert=True); return
    if not await is_group_admin(bot, chat_id, callback.from_user.id):
        await callback.answer("⚠️ Только администратор.", show_alert=True); return
    ...
    if not game.start():
        await callback.answer("❌ Не удалось запустить игру.", show_alert=True); return
    await callback.message.edit_text(
...
async def run_doctor_phase(bot: Bot, game: Game):
    ...
    if game.doctor_target is None: game.doctor_target = random.choice(targets)
...
async def handle_private_target(callback: CallbackQuery, role: str):
    ...
    await send_private_game_message(bot=callback.message.bot, game=game, user_id=uid, text=("🔫 <b>ЦЕЛЬ МАФИИ ВЫБРАНА</b>\n\n" if role == MAFIA else "💊 <b>ЦЕЛЬ ДОКТОРА ВЫБРАНА</b>\n\n") + name, parse_mode="HTML")
    if role == MAFIA:
...
async def publish_vote_results(bot: Bot, game: Game):
    ...
    lines.append("")
    counts=Counter(game.day_votes.values())
    for uid,count in counts.most_common(): lines.append(f"{safe_name(game,uid)} — <b>{count}</b>")
    await send_game_message(bot,game,"\n".join(lines),parse_mode="HTML")
...
async def postgame_end_handler(callback: CallbackQuery, bot: Bot):
    ...
    welcome_messages[chat_id]=welcome.message_id; game_messages[chat_id]=welcome.message_id; game_message_ids.setdefault(chat_id,set()).add(welcome.message_id)
...
async def admin_restart_handler(callback: CallbackQuery, bot: Bot):
    ...
    game_messages[chat_id]=restart_message.message_id; game_message_ids.setdefault(chat_id,set()).add(restart_message.message_id)
...
async def main():
    ...
    commands = [
        BotCommand(command="mafia", description="Начать новую Мафию"),
    ...
    print("🟢 Команды /mafia /stop /restart зарегистрированы")
